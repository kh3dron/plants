...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function generateSunflower() {
    ...
    for (let n = 0; n < N; n++) {
      ...
      const radius = Math.sqrt(n) * 6;
      const x = centerX + radius * Math.cos(angle);
      const y = centerY + radius * Math.sin(angle);
      if (n > 565) {
        // Draw leaf (using the provided design)
        const leafLength = circleSize * 2.5; // scale leaf size to circleSize
        const leafWidth = circleSize * 1.2;
        // Draw leaf stem
        ctx.save();
        ctx.strokeStyle = "#8B5A2B";
        ctx.beginPath();
        ctx.moveTo(x, y);
        // Orient leaf away from center
        const leafAngle = angle; // radians
        const stemEndX = x + Math.cos(leafAngle) * (leafLength * 0.3);
        const stemEndY = y + Math.sin(leafAngle) * (leafLength * 0.3);
        ctx.lineTo(stemEndX, stemEndY);
        ctx.stroke();
        // Draw leaf blade
        ctx.fillStyle = "rgba(34, 139, 34, 0.8)"; // Forest green, 50% opacity
        ctx.beginPath();
        ctx.ellipse(
          stemEndX,
          stemEndY,
